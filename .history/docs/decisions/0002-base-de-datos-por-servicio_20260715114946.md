...
El proyecto está compuesto por `usuarios-service` y `entrenamientos-service`. Era necesario decidir si ambos compartirían una única base de datos MySQL (mediante esquemas o tablas separadas) o si cada servicio sería propietario de su propia base de datos.

## Decisión

Cada microservicio tiene su propia base de datos MySQL, en un contenedor Docker independiente (`usuarios_db` en el puerto 3307, `entrenamientos_db` en el puerto 3308), Cada servicio accede exclusivamente a su propia base de datos. No existe acceso directo a la base de datos del otro servicio.
...
**Trade-offs:**
- Se pierde la posibilidad de hacer `JOIN` entre entidades de distintos servicios. El campo `usuarioId` en `Registro` y `PR` es una referencia lógica (Long), no una clave foránea real.

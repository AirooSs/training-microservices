...
Se optó por una llamada HTTP síncrona (`RestClient`) desde `entrenamientos-service` hacia el endpoint `GET /usuarios/{id}/existe` de `usuarios-service`, esperando la respuesta antes de continuar con la operación.
